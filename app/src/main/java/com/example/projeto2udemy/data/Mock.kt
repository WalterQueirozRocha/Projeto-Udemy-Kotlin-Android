package com.example.projeto2udemy.data

import com.example.projeto2udemy.MotivationConstants
import kotlin.random.Random

class Phrase(val description: String, val caregoryid: Int)

class Mock {

    val sunny = MotivationConstants.FILTER.SUN
    val all = MotivationConstants.FILTER.ALL
    val moon = MotivationConstants.FILTER.MOON

    private val listPhrases: List<Phrase> = listOf(
        Phrase("Não sabendo que era impossível, foi lá e fez.", moon),
        Phrase("Você não é derrotado quando perde, você é derrotado quando desiste!", moon),
        Phrase("Quando está mais escuro, vemos mais estrelas!", moon),
        Phrase("Insanidade é fazer sempre a mesma coisa e esperar um resultado diferente.", moon),
        Phrase("Não pare quando estiver cansado, pare quando tiver terminado.", moon),
        Phrase("O que você pode fazer agora que tem o maior impacto sobre o seu sucesso?", moon),
        Phrase("A melhor maneira de prever o futuro é inventá-lo.", sunny),
        Phrase("Você perde todas as chances que você não aproveita.", sunny),
        Phrase("Fracasso é o condimento que dá sabor ao sucesso.", sunny),
        Phrase(" Enquanto não estivermos comprometidos, haverá hesitação!", sunny),
        Phrase("Se você não sabe onde quer ir, qualquer caminho serve.", sunny),
        Phrase("Se você acredita, faz toda a diferença.", sunny),
        Phrase("Riscos devem ser corridos, porque o maior perigo é não arriscar nada!", sunny)
    )

    fun getPhrase(value: Int): String {
        when (value) {
            sunny -> {
                val filteredList = listPhrases.filter { it.caregoryid == value }
                val index = Random.nextInt(filteredList.size)
                return filteredList[index].description
            }
            moon -> {
                val filteredList = listPhrases.filter { it.caregoryid == value }
                val index = Random.nextInt(filteredList.size)
                return filteredList[index].description
            }
            else -> {
                val index = Random.nextInt(listPhrases.size)
                return listPhrases[index].description
            }
        }

    }
}